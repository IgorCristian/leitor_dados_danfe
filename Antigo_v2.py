...
NOME_ARQUIVO_SAIDA = 'dados_notas_fiscais_DEBUG_NOVOS.xlsx' # Nome para depuração
                ...
                if tabelas_pagina:
                    for tabela in tabelas_pagina:
                         if isinstance(tabela, list) and all(isinstance(row, list) for row in tabela if row is not None):
                             # Limpa Nones DENTRO das linhas ANTES de adicionar
                             tabela_limpa = [[str(cell) if cell is not None else "" for cell in row] for row in tabela if row is not None and any(cell is not None and str(cell).strip() != '' for cell in row)] # Remove linhas totalmente vazias
...
# --- Função Principal de Extração por NF-e (DEBUG INTENSO) ---
def extrair_dados_nf(pdf_path, nome_arquivo):
    """
    Extrai dados com foco em DEPURAR por que os itens não são adicionados.
    Assume layout mesclado UNQVU por padrão (como VFinal) e imprime tudo.
    """
    ...
    print("  DEBUG: Extraindo Cabeçalho...")
    ...
    except Exception as e: print(f"  ERRO Dest: {e}")
    try: # Data Emissão
    ...
    except Exception as e: print(f"  ERRO NFNum: {e}")
    try: # Emitente
    ...
    except Exception as e: print(f"  ERRO Emit: {e}")
    ...
    # --- Extração de Itens (DEBUG - SEM DETECÇÃO DE LAYOUT, ASSUME REGEX UNQVU) ---
    print("\n  DEBUG: Extraindo Itens (Assumindo Layout Mesclado UNQVU para depuração)...")
    dados_itens_finais = []
    tabelas_info = extrair_tabelas_pdf(pdf_path)
    if not tabelas_info: print(f"  DEBUG: Nenhuma tabela encontrada em {nome_arquivo}.")
    else:
        # Regex para Layout A (UN/Qtd/VUnit/VTotal mesclados) - USADO PARA DEBUG
        ...
        for info_tabela in tabelas_info:
            ...
            print(f"\n  DEBUG: Verificando Tabela Pag {pagina_num} - Cabeçalho: {header}")

            # Identifica se parece tabela de itens (simplificado para debug)
            if any('descri' in h or 'produto' in h for h in header) and len(header) > 4:
                print(f"    DEBUG: Tabela Pag {pagina_num} parece ser de itens.")

                # --- MAPEAMENTO SIMPLES (PARA DEBUG) ---
                idx_desc = -1; idx_merged_unqvu = -1
                for i, h_text in enumerate(header): # Descrição
                ...
                # Acha coluna 'UN' para usar como mesclada no teste
                for i, h_text in enumerate(header):
                    if 'un' == h_text.strip(): idx_merged_unqvu = i; break
                if idx_merged_unqvu == -1: # Fallback se não achar 'UN' exato
                     idx_cfop = -1
                     for i, h_text in enumerate(header):
                         if 'cfop' in h_text: idx_cfop = i; break
                     if idx_cfop != -1 and idx_cfop + 1 < len(header): idx_merged_unqvu = idx_cfop + 1
                     else: idx_merged_unqvu = 5 # Fallback final
                print(f"      DEBUG Índices -> Desc:{idx_desc}, MergedUNQVU:{idx_merged_unqvu}")

                if idx_desc == -1 or idx_merged_unqvu == -1: print(f"      DEBUG: Não mapeou Desc ou Coluna Mesclada."); continue

                # --- Processa as Linhas (ASSUMINDO REGEX UNQVU) ---
                print(f"      DEBUG: Processando {len(tabela)-1} linhas...")
                for num_linha, linha_bruta in enumerate(tabela[1:], start=1):
                    print(f"\n      DEBUG Linha {num_linha} Bruta: {linha_bruta}")
                    # Garante que a linha tenha colunas suficientes
                    if len(linha_bruta) > max(idx_desc, idx_merged_unqvu):
                        ...
                        merged_cell_text = get_cell_value(linha_bruta, idx_merged_unqvu)
                        print(f"        DEBUG Desc: '{item_desc}'")
                        print(f"        DEBUG Texto Célula Mesclada (Índice {idx_merged_unqvu}): '{merged_cell_text}'")

                        quantidade = None; valor_unitario = None; valor_total_item = None
                        qtd_str = None; vunit_str = None; vtotal_str = None

                        if merged_cell_text:
                            match = regex_unqvu.search(merged_cell_text)
                            if match:
                                print(f"        DEBUG Regex UNQVU Casou! Grupos: {match.groups()}")
                                # group(1) é UN
                                qtd_str = match.group(2); quantidade = limpar_numero(qtd_str)
                                vunit_str = match.group(3); valor_unitario = limpar_numero(vunit_str)
                                vtotal_str = match.group(4); valor_total_item = limpar_numero(vtotal_str)
                                print(f"          -> Qtd: {qtd_str} -> {quantidade}")
                                print(f"          -> VUnit: {vunit_str} -> {valor_unitario}")
                                print(f"          -> VTotItem: {vtotal_str} -> {valor_total_item}")
                            else:
                                print(f"        DEBUG Regex UNQVU NÃO casou para '{merged_cell_text}'")
                                # >>> ADICIONA TENTATIVA COM OUTRO REGEX (QVU) COMO TESTE <<<
                                regex_qvu = re.compile(r"\(?([\d.,:]+)\)?\s*\(?([\d.,:]+)\)?\s*\(?([\d.,:]+)\)?")
                                match_qvu = regex_qvu.search(merged_cell_text)
                                if match_qvu:
                                     print(f"        DEBUG Regex QVU Casou! Grupos: {match_qvu.groups()}")
                                     qtd_str = match_qvu.group(1); quantidade = limpar_numero(qtd_str)
                                     vunit_str = match_qvu.group(2); valor_unitario = limpar_numero(vunit_str)
                                     vtotal_str = match_qvu.group(3); valor_total_item = limpar_numero(vtotal_str)
                                     print(f"          -> Qtd: {qtd_str} -> {quantidade}")
                                     print(f"          -> VUnit: {vunit_str} -> {valor_unitario}")
                                     print(f"          -> VTotItem: {vtotal_str} -> {valor_total_item}")
                                else:
                                     print(f"        DEBUG Regex QVU também NÃO casou para '{merged_cell_text}'")
                        else:
                             print(f"        DEBUG Célula mesclada (Índice {idx_merged_unqvu}) vazia.")

                        # Verifica se conseguiu extrair dados essenciais
                        if item_desc and quantidade is not None and valor_unitario is not None:
                            print(f"        --->>> DEBUG: ITEM VÁLIDO PARA ADIÇÃO <<<---")
                            ...
                            print(f"        --->>> DEBUG: ITEM INVÁLIDO, NÃO SERÁ ADICIONADO (Desc:'{item_desc}', Qtd:{quantidade}, VUnit:{valor_unitario}) <<<---")
                    else:
                         print(f"      DEBUG Linha {num_linha} pulada (curta ou vazia)")

    # --- Fallback Final ---
    if not dados_itens_finais:
         print(f"\n  AVISO FINAL: Nenhum item válido extraído de {nome_arquivo}.")
         dados_itens_finais.append({
            'Arquivo Origem': nome_arquivo,
            'Nome Emitente': nome_emitente, 'Data Emissão': data_emissao, 'Número NF': numero_nf,
            'Destinatário': destinatario, 'Valor Total NF': valor_total_nota,
            'Item Descrição': 'NENHUM ITEM EXTRAÍDO (DEBUG)',
...
def processar_pasta_pdfs(pasta_raiz, arquivo_saida):
    ...
    for pdf_path in arquivos_pdf:
        ...
        dados_da_nf_atual = extrair_dados_nf(pdf_path, nome_arquivo_pdf) # Chama a função de debug
    ...
    if df_filtrado.empty: print("AVISO: Nenhum item válido extraído. Nenhum arquivo Excel será salvo."); return
    df_final = df_filtrado
    print(f"Salvando {len(df_final)} linhas válidas...")
    colunas_ordenadas = [
        'Arquivo Origem', 'Nome Emitente', 'Data Emissão', 'Número NF', 'Destinatário',
